...
const PayrollReport = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const data = await getPayrollEntries(selectedMonth);
        setPayrollData(data);
        setFilteredData(data);
        
        // Extract unique months from data
        const uniqueMonths = [...new Set(data.map(entry => {
          const date = new Date(entry.report_month);
  ...
  useEffect(() => {
    if (searchQuery.trim() === '') {
      ...
      const filtered = payrollData.filter(entry => 
        ...
        entry.card_no.toLowerCase().includes(query)
      );
  ...
  return (
    <div className="p-6">
      <h2 className="text-2xl font-semibold mb-6">Payroll Report</h2>
